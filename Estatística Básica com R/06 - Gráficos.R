...
barplot(vetor_total_resultados, col = c(1,2,3,4,5)) #mudar as cores das colunas

#Salvando o gráfico em disco
png("Barplot.png", width = 480, height = 480)
barplot(vetor_total_resultados,
        col = rgb(0.5,0.1,0.6,0.6), #cor das colunas
        xlab = "Categorias", #rótulo do eixo x
        ylab = "Valores", #rótulo do eixo y
        main = "Barplot em R", #título do gráfico
        ylim = c(0,60)) #tamanho da escala do gráfico
dev.off()

###ggplot2###
library(ggplot2)
View(mtcars) #dataset vem junto com R
...
pie(fatias, labels = paises, main = "Leituras de Livros por Pessoa/Ano")
...
fatias <- c(4, 12, 14, 16, 8)
paises <- c("Brasil", "Estados Unidos", "Alemanha", "Reino Unido", "Espanha")
pie3D(fatias, labels = paises, explode = 0.1, main = "Leituras de Livros por Pessoa/Ano")
...
ggplot(data, aes(x = my_x, y = my_y)) +
  geom_point(shape = 1) +
  geom_smooth(method = lm, color = "red", se = TRUE) #adiciona margem de erro
...
dadostree = data.frame(grupo, subgrupo, valor)
print(dadostree)
